...
const memoriesEmail = async ({ tags, year }: any) => {
  ...
  if (members.length > 0) {
    ...
    const title = `Celebrating ${year} Year of Cherished Moments at ${caption} with ${member}`;
          ...
          <tbody>
            <tr>
              <td>
                ...
                <p data-id="react-email-text" style="font-size:14px;line-height:22px;margin:16px 0;color:#3c4043">${title}</p>
              </td>
            </tr>
          </tbody>
        </table>
    ...
    const memberEmP = await members.map(
      (set: any) => set.tag.memberPersonalEmail
    );
    await sendGridEmail({
