...
import { useSortTest } from "@apolloo/actions";
import ApiLoading from "@components/Loading/ApiLoading";
const Items: React.FC = ({ data, loading }: any) => {
  const [sort, { error: err2, loading: loading3 }] = useSortTest();
  const set = data.getTestimonial
    .filter((set) => set.status === true)
    .sort((a, b) => a.sort - b.sort);
  var result = set.map((t) => ({
    name: t.id,
    description: t.testimonialName,
    img: t.testimonialImage,

    memberDesignation: t.projectName,
  }));
  ...
  const data1 = useMemo(() => data.getTestimonial, []);
